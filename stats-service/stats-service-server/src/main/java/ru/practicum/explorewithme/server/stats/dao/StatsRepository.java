package ru.practicum.explorewithme.server.stats.dao;
...
public interface StatsRepository extends JpaRepository<EndpointHit, Integer> {
    ...
    @Query("""
            select new ru.practicum.explorewithme.server.stats.model.ViewStats(eh.app, eh.uri, count(distinct eh.ip))
    ...
    @Query("""
            select new ru.practicum.explorewithme.server.stats.model.ViewStats(eh.app, eh.uri, count(eh.ip))
            from EndpointHit as eh
            where eh.timestamp between ?1 and ?2
            and (?3 is null or eh.uri in ?3)
            group by eh.app, eh.uri
